...
type FileError = { _tag: 'FileError'; path: string; operation: string; cause?: Error }
type ValidationError = { _tag: 'ValidationError'; field: string; reason: string }
type NetworkError = { _tag: 'NetworkError'; url: string; status?: number }

type AppError = FileError | ValidationError | NetworkError

// Pattern matching helper
function matchError<R>(error: AppError, patterns: {
...
export const tapOk = <T>(fn: (value: T) => void) => (result: Result<T>): Result<T> => {
  if (result.success) fn(result.value)
  return result
}

export const tapErr = <E>(fn: (error: E) => void) => (result: Result<any, E>): Result<any, E> => {
