...
```typescript
const parseAuto = (filePath: string): Promise<DataResult<any>> => {
```
...
```typescript
const parseAutoFromContent = (content: string, fileName?: string): Promise<DataResult<any>> => {
```
...
```typescript
const writeAuto = (filePath: string, data: any): Promise<Result<void, CoreError>> => {
```
...
```typescript
const detectFormat = (filePath: string): Promise<Result<string, CoreError>> => {
```
...
```typescript
const detectFormatFromContent = (content: string, fileName?: string): Result<string, CoreError> => {
```
...
```typescript
const convertFormat = (data: any, targetFormat: 'csv' | 'json' | 'excel'): Result<string, CoreError> => {
```
...
```typescript
const createUnifiedDataOperations = (config?: UnifiedDataConfig): UnifiedDataOperations => {
```
...
```typescript
const createCSVOperations = (config?: DataConfig): CSVOperations => {
```
...
```typescript
const createJSONOperations = (config?: DataConfig): JSONOperations => {
```
...
```typescript
const createExcelOperations = (config?: DataConfig): ExcelOperations => {
```
...
```typescript
const createDetectionOperations = (config?: FormatConfig): DetectionOperations => {
```
...
```typescript
const createMimeOperations = (config?: FormatConfig): MimeOperations => {
```
...
```typescript
const createConversionOperations = (config?: FormatConfig): ConversionOperations => {
```
...
const mapNodeError = (operation: string, path: string, error: unknown): CoreError => {

// Library error mapping
const mapLibraryError = (library: string, operation: string, error: unknown): CoreError => {

// Validation error mapping
const mapValidationError = (field: string, value: unknown, error: unknown): CoreError => {
```
...
- [FileSystem API](../../../fs/reference/api.md) - File operations used internally
