...
Specific error types used across the packages.

### CoreError

Base error type from `@esteban-url/core`:
...
Error type for file operations from `@esteban-url/fs`:
...
interface FileSystemError extends Error {
  ...
  message: string // Error description
  code: string // System error code (e.g., 'ENOENT', 'EACCES', 'EISDIR')
  path?: string // File path that caused the error
  syscall?: string // System call that failed (e.g., 'open', 'unlink')
...
Error type for configuration operations from `@esteban-url/config`:

```typescript
import type { ConfigError } from '@esteban-url/config'

interface ConfigError extends Error {
  name: 'ConfigError'
  message: string // Error description
  code: ConfigErrorCode // Specific error code
  source?: string // Which config source failed
  path?: string // Config key path that failed
...
function wrapError(original: Error, context: string): CoreError {
  return {
    name: 'CommandError',
...
// 3. Propagate errors with additional context
const configResult = await loadConfig()
if (configResult.isErr()) {
  return err(wrapError(configResult.error, 'Failed to load configuration'))
}

// 4. Use fromThrowable for unsafe operations
import { fromThrowable } from '@esteban-url/core'

const safeParse = fromThrowable(JSON.parse)
const result = safeParse(jsonString)
if (result.isErr()) {
  logger.error(`JSON parse failed: ${result.error.message}`)
}
```
...
The CLI provides structured logging utilities through consola.

```typescript
// Structured logging (recommended approach)
import { logSuccess, logError, logWarning, logInfo } from '@esteban-url/cli/utils'
...
// Direct consola usage
import { consola } from '@esteban-url/cli/utils'
...
// Raw colors when needed (less common)
import { colors } from '@esteban-url/cli/utils'

console.log(colors.green('Custom green text'))
console.log(colors.red('Custom red text'))
console.log(colors.bold('Bold text'))
```

## Logging

Structured logging with consola-based logger.

### Logger Interface

Available on command context. Uses consola for cross-platform colored output.

```typescript
interface Logger {
  info: (message: string, ...args: unknown[]) => void // Informational messages
  success: (message: string, ...args: unknown[]) => void // Success messages with checkmark
  warning: (message: string, ...args: unknown[]) => void // Warning messages with symbol
  error: (message: string, error?: unknown) => void // Error messages with symbol
  debug: (message: string, ...args: unknown[]) => void // Debug (verbose mode only)
  step: (message: string, ...args: unknown[]) => void // Progress messages with arrow
  raw: ConsolaInstance // Raw consola for advanced usage
}
...
action: async (options, context) => {
  ...
  logger.success('Operation complete') // Shows with ✓
  logger.error('Operation failed', error)
  logger.warning('Potential issue') // Shows with ⚠
  logger.step('Step 1 of 3...') // Shows with →
}

// Creating a logger
import { createDefaultLogger } from '@esteban-url/cli/utils'

const logger = createDefaultLogger(true) // verbose=true shows debug messages
...
Type-safe data processing with automatic format detection.

### @esteban-url/data

Unified data processing for CSV, JSON, and Excel formats.
...
if (result.isOk()) {
  console.log(result.value.data) // Parsed data
  console.log(result.value.format) // Detected format
}
...
- **CSV**: Customizable delimiters, headers, quotes
- **JSON**: Minification, formatting, validation
- **Excel**: Multi-sheet support, formatting preservation

### @esteban-url/sort

Fast, type-safe sorting utilities.
...
import { sortStrings } from '@esteban-url/sort'
const sorted = sortStrings(['file10.txt', 'file2.txt', 'file1.txt'])
// => ['file1.txt', 'file2.txt', 'file10.txt']
```

### @esteban-url/validation

Comprehensive validation utilities with Result types.

```typescript
import { validate, z } from '@esteban-url/validation'
...
// Validate with Result types
import { createSchemaValidator } from '@esteban-url/validation'

const validateUser = createSchemaValidator(userSchema)
const result = await validateUser(userData)
if (result.isOk()) {
  // TypeScript knows result.value matches userSchema
...
- **Strings**: email, URL, phone, length, patterns
- **Numbers**: range, positive, integer, currency
- **Dates**: format, range, age
- **Arrays**: length, items, uniqueness
- **Objects**: nested validation, required fields

#### Schema Composition

```typescript
import { emailSchema, mergeSchemas, withDefault } from '@esteban-url/validation'

// Compose complex schemas
const profileSchema = mergeSchemas(emailSchema, { username: z.string() })

// Add defaults
const configSchema = withDefault({ port: z.number() }, { port: 3000 })
```
...
interface CommandContext {
  ...
  logger: Logger // Consola-based logger instance (see Logger interface above)
...
// Interactive prompts (re-exports @inquirer/prompts plus utilities)
import { input, confirm, select, checkbox, password, editor } from '@esteban-url/cli/prompts'
import { createConfirmationPrompt, createDirectoryPrompt } from '@esteban-url/cli/prompts'

// Utilities for CLI operations
import { createDefaultLogger, createSpinner, withSpinner } from '@esteban-url/cli/utils'
import { consola, colors, logSuccess, logError, logWarning, logInfo } from '@esteban-url/cli/utils'
import {
  detectPackageManager,
  getRunCommand,
  execPackageManagerCommand,
} from '@esteban-url/cli/utils'
import type { Logger, PackageManager } from '@esteban-url/cli/utils'

// Progress tracking
import { createProgressBar, createMultiProgressBar } from '@esteban-url/cli/progress'
import { createProgressTracker, updateProgress } from '@esteban-url/cli/progress'
import type { ProgressBar, ProgressTracker } from '@esteban-url/cli/progress'

// Testing utilities
import { createTestContext, runCLICommand } from '@esteban-url/cli/testing'
import { createMockLogger, createMockFS } from '@esteban-url/cli/testing'
import type { TestContext, MockLogger, MockFS } from '@esteban-url/cli/testing'
```
