...
const addItem = (array: string[], item: string): string[] => {
...
async const processOrder = async (orderId: string): Promise<Result<Order>> => {
...
async const getConfig = async (cache: Cache, fs: FileSystem) => {
...
- [Testing Best Practices](../testing-guide)
