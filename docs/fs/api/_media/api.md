...
```typescript
const readFile = (config?: FSConfig): ReadFileOp => {
```
...
```typescript
const writeFile = (config?: FSConfig): WriteFileOp => {
```
...
```typescript
const exists = (config?: FSConfig): ExistsOp => {
```
...
```typescript
const stat = (config?: FSConfig): StatOp => {
```
...
```typescript
const mkdir = (config?: FSConfig): MkdirOp => {
```
...
```typescript
const readDir = (config?: FSConfig): ReadDirOp => {
```
...
```typescript
const copy = (config?: FSConfig): CopyOp => {
```
...
```typescript
const move = (config?: FSConfig): MoveOp => {
```
...
```typescript
const remove = (config?: FSConfig): RemoveOp => {
```
...
```typescript
const readJson = (config?: FSConfig): ReadJsonOp => {
```
...
```typescript
const writeJson = (config?: FSConfig): WriteJsonOp => {
```
...
```typescript
const ensureDir = (config?: FSConfig): (path: string) => Promise<FSResult<void>> => {
```
...
```typescript
const outputFile = (config?: FSConfig): (path: string, content: string) => Promise<FSResult<void>> => {
```
...
```typescript
const emptyDir = (config?: FSConfig): (path: string) => Promise<FSResult<void>> => {
```
...
```typescript
const readIfExists = (config?: FSConfig): (path: string) => Promise<FSResult<string | null>> => {
```
...
```typescript
const mapNodeError = (operation: string, path: string, error: unknown): FileSystemError => {
```
...
For detailed documentation of filesystem operations, see the [file operations guide](../../how-to/file-operations.md).
...
For detailed testing documentation, see the testing section in the [package README](../../README.md).
