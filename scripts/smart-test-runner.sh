...
main() {
  ...
  if [[ "$FORCE_TESTS" == "true" ]]; then
    execute_tests "$test_command" "🔴 Forced full test suite execution"
    return $?
  fi
  ...
  case "$risk_level" in
    "high")
      execute_tests "$test_command" "🔴 Code changes detected - running full test suite"
      return $?
      ;;
    "medium")
      # Get affected packages
      ...
      ;;
    "skip")
      log_success "🟢 Documentation/config changes only - skipping tests"
      return 0
      ;;
    *)
      log_warning "Unknown risk level: '$risk_level', running full test suite"
      execute_tests "$test_command" "🟡 Running full test suite (unknown risk level)"
      return $?
      ;;
  esac
}
