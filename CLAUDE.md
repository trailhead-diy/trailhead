...
- `gbcp` = Git branch, commit, push/PR workflow: 1) Create new feature branch from current branch, 2) Stage all changes and commit with meaningful message (gcwm), including "Closes #[issue-number]" or "Fixes #[issue-number]" in commit message if working on a GitHub issue, 3) Push branch and create PR with auto-generated description (gpr), ensuring PR description includes "Closes #[issue-number]" to auto-link and close the issue when merged.
...
- `gh-issue` = Comprehensive GitHub issue analysis and implementation planning: 1) Fetch issue details with `gh issue view [number] --comments` to get full conversation history, 2) Analyze ALL comments for additional context, requirements changes, design decisions, and implementation hints, 3) Create feature branch from main with semantic naming (feat/fix/refactor-issue-number-brief-description), 4) IMMEDIATELY checkout the new branch to ensure all work happens in the feature branch and main remains clean. 5) Deep codebase examination using search tools to identify: existing partial implementations, related code patterns, similar features, affected dependencies, and architectural constraints, 6) Apply core principles analysis: functional patterns, type safety, DRY violations to avoid, YAGNI considerations, and high-ROI testing approach, 7) Cross-reference with related issues/PRs using `gh issue list` and `gh pr list` for context, 8) Identify monorepo impact across all packages and apps, 9) Create comprehensive TodoWrite plan with: problem analysis summary, codebase findings, implementation strategy aligned with principles, specific atomic tasks, testing approach, and affected package considerations.
