...
export function CatalystText({
  ...
  const resolvedStyles =
    color && isSemanticToken(color) ? createSemanticTextStyles(color) : '';
  return (
    <p
      data-slot="text"
      {...props}
...
export function CatalystStrong({
  className,
  ...props
...
export function CatalystCode({
  ...
  return (
    <code
      {...props}
      className={cn(
        'rounded-sm border border-border bg-card/2.5 px-0.5 text-sm font-medium text-foreground sm:text-[0.8125rem] dark:border-border dark:bg-card/2.5 dark:text-foreground',
        className,
      )}
    />
  );
