...
export class DemoPage {
  ...
  async switchTheme(themeName: string) {
    ...
    const switcher = await this.page.$('[role="combobox"][aria-label*="theme" i], button:has-text("Theme"), button[aria-label*="theme" i]')
    ...
    if (switcher) {
      ...
      await this.page.waitForSelector('[role="option"], [role="listbox"] button', { state: 'visible' })
      
      // Click the theme option
      await this.page.click(`[role="option"]:has-text("${themeName}"), [role="listbox"] button:has-text("${themeName}")`)
    } else {
