...
export function generateMarkdownReport(
  ...
  lines.push(`| Throughput | ${comparison.transforms2.componentsPerSecond.toFixed(1)} components/sec |`)
  lines.push('')
  ...
  if (comparison.traditional && comparison.speedupFactor) {
    ...
    lines.push(`| Throughput | ${comparison.traditional.componentsPerSecond.toFixed(1)} components/sec |`)
    lines.push('')
    ...
    lines.push('|--------|---------|-------------|-------------|')
    lines.push(`| Average Time | ${formatDuration(comparison.transforms2.averageTime)} | ${formatDuration(comparison.traditional.averageTime)} | **${comparison.speedupFactor.toFixed(1)}x faster** |`)
    
    const memoryImprovement = comparison.memoryEfficiency! > 0 
      ? `**${comparison.memoryEfficiency!.toFixed(0)}% less**`
      : `${Math.abs(comparison.memoryEfficiency!).toFixed(0)}% more`
    lines.push(`| Memory Usage | ${formatMemory(comparison.transforms2.memoryAverage)} | ${formatMemory(comparison.traditional.memoryAverage)} | ${memoryImprovement} |`)
    
    const throughputImprovement = ((comparison.transforms2.componentsPerSecond / comparison.traditional.componentsPerSecond - 1) * 100)
    lines.push(`| Throughput | ${comparison.transforms2.componentsPerSecond.toFixed(1)}/sec | ${comparison.traditional.componentsPerSecond.toFixed(1)}/sec | **${throughputImprovement.toFixed(0)}% higher** |`)
    lines.push('')
    ...
    lines.push(`Primary:     ${'█'.repeat(t2Bars)} ${formatDuration(comparison.transforms2.averageTime)}`)
    lines.push(`Traditional: ${'█'.repeat(tradBars)} ${formatDuration(comparison.traditional.averageTime)}`)
    lines.push('```')
    lines.push('')
  }
  ...
  if (comparison.speedupFactor && comparison.speedupFactor > 10) {
    lines.push(`The primary approach demonstrates exceptional performance, running **${comparison.speedupFactor.toFixed(1)}x faster** than the traditional approach. This significant improvement is achieved through:`)
    lines.push('')
    lines.push('1. **Modular Transform Architecture**: Each transform is focused and optimized')
    lines.push('2. **Efficient Pattern Matching**: Using targeted patterns instead of broad searches')
  ...
  if (comparison.transforms2.componentProfiles.length > 0) {
    ...
    comparison.transforms2.componentProfiles
      ...
      .slice(0, 10)
      .forEach(comp => {
        lines.push(`| ${comp.name} | ${formatDuration(comp.executionTime)} | ${formatMemory(comp.memoryUsed)} | ${comp.transformCount} |`)
      })
  ...
  if (comparison.transforms2.averageTime < 100) {
    ...
    lines.push('2. 📊 **Profile Individual Transforms**: Identify which transforms take the most time')
  }
