 ...
 */
function applyDefaultColorsResolution(
  root: any,
  j: any,
  variableName: string
): void {
...
export function needsResolution(
  ...
  const hasVariable = j(functionBody).find(j.VariableDeclarator, {
  ...
  if (hasColorsObject) {
    const usesColors = j(functionBody).find(j.MemberExpression, {
      object: { name: 'colors' }
    }).length > 0
  ...
  return j(functionBody).find(j.JSXAttribute, {
    name: { name: 'className' }
  }).length > 0
}