/**
 * Transform to update Checkbox component default color from 'dark/zinc' to 'primary'
 *
 * This transform specifically targets the default value in the Checkbox component
...
/**
 * Update Checkbox default color transform
 */
export const checkboxDefaultColorTransform: Transform = {
  name: 'checkbox-default-color-update',
  description: 'Update Checkbox component default color from dark/zinc to primary',
  type: 'ast',
  ...
  execute(content: string): TransformResult {
    ...
    if (!content.includes('Checkbox') || !content.includes("color = 'dark/zinc'")) {
      return {
        content,
        changes: [],
        hasChanges: false,
    ...
    try {
      ...
      const transformer = (fileInfo: FileInfo, _api: API) => {
        ...
        // Find the Checkbox function declaration
        root
      ...
      console.error('Error in Checkbox default color transform:', error);
      return {
        content,
        changes: [
          {
