...
export interface MockValidator {
  ...
  createCompositeSchema(schemaIds: string[], composition: 'merge' | 'intersection' | 'union'): Result<ValidationSchema, CoreError>
  mockValidationResult(schemaId: string, data: any, result: ValidationResult): void
  getValidationHistory(): Array<{ schemaId: string; data: any; result: ValidationResult; timestamp: number }>
  clearMocks(): void
}
...
export function createMockValidator(): MockValidator {
  ...
  const validationHistory: Array<{ schemaId: string; data: any; result: ValidationResult; timestamp: number }> = []
  ...
  return {
      ...
      try {
        ...
        for (const rule of schema.rules) {
          ...
          if (rule.min !== undefined && getLength(value) < rule.min) {
            errors.push({
              field: rule.field,
              message: rule.message || `Field '${rule.field}' must be at least ${rule.min} characters/items`,
              value,
              rule: 'min',
            })
          }
          ...
          if (rule.max !== undefined && getLength(value) > rule.max) {
            errors.push({
              field: rule.field,
              message: rule.message || `Field '${rule.field}' must be at most ${rule.max} characters/items`,
              value,
              rule: 'max',
            })
          }
          ...
          if (rule.custom) {
            ...
            if (customResult !== true) {
              errors.push({
                field: rule.field,
                message: typeof customResult === 'string' ? customResult : (rule.message || `Field '${rule.field}' failed custom validation`),
                value,
                rule: 'custom',
              })
            }
    ...
    createCompositeSchema(schemaIds: string[], composition: 'merge' | 'intersection' | 'union'): Result<ValidationSchema, CoreError> {
      const schemasList = schemaIds.map(id => schemas.get(id)).filter(Boolean) as ValidationSchema[]
  ...
  schemas: {
    user: {
      ...
      rules: [
        ...
        { field: 'role', type: 'string' as const, required: false, custom: (value: string) => ['user', 'admin', 'moderator'].includes(value) },
      ],
      strict: true,
      allowUnknown: false,
    ...
    config: {
      ...
      rules: [
        ...
        { field: 'app.version', type: 'string' as const, required: true, pattern: /^\d+\.\d+\.\d+$/ },
...
export function assertValidationErrors<T>(
  ...
  for (const expectedError of expectedErrors) {
    const matchingError = validation.errors.find(e => 
      e.field === expectedError.field &&
      (!expectedError.rule || e.rule === expectedError.rule)
    )
 ...
 */
export function createValidationTestScenario(options: {
  ...
  testZodValidation: <T>(zodSchema: z.ZodSchema<T>, data: any) => Result<ValidationResult<T>, CoreError>
  testComposition: (schemaIds: string[], composition: 'merge' | 'intersection' | 'union', data: any) => Result<ValidationResult, CoreError>
  cleanup: () => void
} {
  const validator = createMockValidator()
