 ...
 *
 * @param variant - Template variant identifier (deprecated, always uses 'basic')
 ...
 *
 * Resolution priority:
 * 1. Custom base directory from config.templatesDir
 * 2. Built-in template directories (default)
 *
 * @example
 * ```typescript
 * const { paths, dirs } = resolveTemplatePaths('basic', {
 *   templatesDir: '/custom/templates'
 * })
 * // dirs.variant will be '/custom/templates/basic'
...
export function resolveTemplatePaths(variant: string, config?: TemplateLoaderConfig) {
  ...
  const variantDir = join(baseTemplatesDir, variant)
