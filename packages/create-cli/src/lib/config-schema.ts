...
export const modernProjectConfigSchema = z.object({
  ...
  template: templateVariantSchema,
 ...
 */
export function validateModernProjectConfig(
  config: unknown
...
/**
 * Create a configuration file from ModernProjectConfig
 */
export function createConfigFile(config: ModernProjectConfig): ConfigFile {
  return {
...
): Partial<ModernProjectConfig> {
  return {
    ...userConfig,
    template: preset.template, // Preset always overrides user for template
    ...
    initGit: userConfig.initGit ?? preset.initGit,
