...
export function parseArguments(
  args: string[]
): Result<Partial<ProjectConfig> & { help: boolean; version: boolean }, CoreError> {
  try {
    const config: Partial<ProjectConfig> & { help: boolean; version: boolean } = {
      projectName: '',
      projectPath: '',
      packageManager: 'pnpm',
      includeDocs: false,
    ...
    while (i < args.length) {
      ...
      switch (arg) {
        ...
        case '-t':
        case '--template':
          // Template is deprecated, skip the value if provided
