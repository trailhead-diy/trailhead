 ...
 *
 * File format testing utilities for format detection, conversion testing, and format validation.
 * Provides domain-focused utilities for testing format operations and transformations.
 *
 * @example
 * ```typescript
 * import {
 ...
 * } from '@esteban-url/formats/testing'
 * 
 * // Test format detection
 * const detector = createMockFormatDetector()
 * const result = await detector.detectFormat('data.json')
 * assertFormatDetection(result, 'json')
 * 
 * // Test format conversion
 * const conversion = await testFormatConversion(
 *   formatFixtures.csv.simple,
 *   'csv',
 *   'json'
 * )
 * ```
...
export interface MockFormatDetector {
  ...
  detectFormatFromContent(content: string, fileName?: string): Result<FormatDetectionResult, CoreError>
...
export function assertFormatDetection(
  ...
  if (detection.confidence < minimumConfidence) {
    throw new Error(
      `Expected confidence >= ${minimumConfidence}, but got ${detection.confidence}`
    )
  }
}
