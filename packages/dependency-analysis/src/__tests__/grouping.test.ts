...
describe('Analysis Mode Detection', () => {
  ...
  it('should use complex mode when threshold is exceeded', () => {
    ...
    expect(determineAnalysisMode(changes)).toBe('complex')
    // Even with higher threshold, 15 files is still complex because isSimpleChange returns false for > 10 files
    expect(determineAnalysisMode(changes, { complexityThreshold: 20 })).toBe('complex')

    // Test with fewer files that would be simple
    const smallChanges = createMockChanges(8)
    expect(determineAnalysisMode(smallChanges)).toBe('simple')
    expect(determineAnalysisMode(smallChanges, { complexityThreshold: 5 })).toBe('complex')
  })
})
