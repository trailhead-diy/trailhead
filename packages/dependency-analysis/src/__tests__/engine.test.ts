...
describe('DependencyAnalysisEngine', () => {
  ...
  it('should analyze complex changes with dependency graph', async () => {
    ...
    // Create nodes for all the mock changes
    const nodes = new Map()
    const edges = new Map()
    changes.forEach((change) => {
      nodes.set(
        ...
        createMockDependencyNode({
          ...
          apiSurfaceChanges: change.affectsPublicAPI,
        })
      )
      edges.set(change.path, [])
    })

    vi.mocked(depCruiser.generateDependencyGraphWithCruiser).mockResolvedValue(
      ok(createMockDependencyGraph({ nodes, edges, roots: [changes[0].path] }))
    )

    vi.mocked(treeSitter.parseTypeScriptModule).mockResolvedValue(
      ok({
        imports: [],
        exports: [{ name: 'testExport', isTypeOnly: false, isDefault: false, line: 1 }],
        hasApiChanges: true,
      })
    )
  ...
  it('should respect analysis options', async () => {
    ...
    const edges = new Map()
    changes.forEach((change) => {
      nodes.set(change.path, createMockDependencyNode({ path: change.path }))
      edges.set(change.path, [])
    })

    vi.mocked(depCruiser.generateDependencyGraphWithCruiser).mockResolvedValue(
      ok(createMockDependencyGraph({ nodes, edges }))
    )
  ...
  it('should include warnings for circular dependencies', async () => {
    ...
    vi.mocked(depCruiser.generateDependencyGraphWithCruiser).mockResolvedValueOnce(
      ok(
        createMockDependencyGraph({
          nodes,
          edges,
    ...
    expect(result.isOk()).toBe(true)
    if (result.isOk()) {
      expect(result.value.warnings).toContainEqual(expect.stringContaining('circular dependencies'))
    }
  })
  ...
  it('should include warnings for high-risk groups', async () => {
    ...
    // Mock dependency graph for high risk change
    const nodes = new Map()
    nodes.set(
      'src/api.ts',
      createMockDependencyNode({
    ...
    vi.mocked(depCruiser.generateDependencyGraphWithCruiser).mockResolvedValue(
      ok(createMockDependencyGraph({ nodes, edges: new Map([['src/api.ts', []]]) }))
    )

    vi.mocked(treeSitter.parseTypeScriptModule).mockResolvedValue(
      ok({
        imports: [],
        exports: [{ name: 'apiExport', isTypeOnly: false, isDefault: false, line: 1 }],
        hasApiChanges: true,
      })
    )
    ...
    expect(result.isOk()).toBe(true)
    if (result.isOk()) {
      const hasHighRiskWarning = result.value.warnings.some((w: string) => w.includes('high-risk'))
