...
import { topologicalSort, findDependents } from "../graph/index.js";
...
function groupByDependencyDepth(
  ...
  for (const depth of sortedDepths) {
    ...
    groups.push({
      ...
      dependencies: depthChanges.flatMap(c => 
        context.graph.nodes.get(c.path)?.compilationDependencies || []
      ),
...
export function groupByRiskAndDependency(
  ...
  const deletions = changes.filter(c => c.type === "deletion" && !context.excludedFiles.has(c.path));
  ...
  const apiChanges = changes.filter(c => 
    c.affectsPublicAPI && 
    !processedFiles.has(c.path) && 
    !context.excludedFiles.has(c.path)
  );
  ...
  const dependentPaths = findDependents(graph, apiChanges.map(c => c.path));
  const dependentChanges = dependentPaths
    .filter(p => context.changes.has(p) && !processedFiles.has(p) && !context.excludedFiles.has(p));
