...
function isCoreImplementation(path: string): boolean {
  return (
    (!isTestFile(path) && !isInfrastructure(path) && path.endsWith('.ts')) ||
    path.endsWith('.tsx') ||
    path.endsWith('.js') ||
    path.endsWith('.jsx')
  )
...
function createTestGroup(changes: readonly FileChange[]): AtomicCommitGroup {
  ...
  const _modifications = changes.filter((c) => c.type === 'modification')
...
function createSingleFileGroup(change: FileChange, index: number): AtomicCommitGroup {
  const typePrefix =
    change.type === 'addition' ? 'feat' : change.type === 'deletion' ? 'refactor' : 'fix'
...
function createInfraGroup(changes: readonly FileChange[]): AtomicCommitGroup {
  ...
  const hasLockFile = changes.some(
    (c) =>
...
export function detectSimpleGroups(
