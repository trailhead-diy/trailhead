...
function createFileChange(path: string, type: FileChange["type"]): FileChange {
  const hasImportChanges = path.endsWith(".ts") || 
                         path.endsWith(".tsx") || 
                         path.endsWith(".js") || 
                         path.endsWith(".jsx");
  
  const affectsPublicAPI = path.includes("/src/") && 
                          !path.includes("/test/") && 
...
async function createCommit(
  ...
  const message = options.conventionalCommitFormat !== false 
    ? group.description 
...
export async function createAtomicCommits(
  ...
  const rollback = async (error: DependencyError): Promise<Result<readonly CommitResult[], DependencyError>> => {
  ...
  for (const group of groups) {
    ...
    if (options.validateEachCommit && group.validationCommands.length > 0) {
      ...
      validationPassed = await validateCommit(
        group.validationCommands,
        git.repository
      );
