...
describe("Greet Command", () => {
  ...
  it("should greet user", async () => {
    ...
    const result = await greetCommand.execute({ name: "Alice" }, context);
...
describe("Interactive Installation", () => {
  it("should guide user through installation", async () => {
    ...
    const result = await installCommand.execute({ interactive: true }, context);
...
describe("Error Recovery", () => {
  ...
  it("should rollback on partial failure", async () => {
    ...
    const operations = [
      ...
      {
        name: "op3",
        execute: async () => err(createError({ message: "Failed" })),
      },
    ];
