...
const deleteCommand = createCommand({
  ...
  options: [{ name: "pattern", alias: "p", type: "string", required: true }],
  action: async (options, context) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!result.success) {
        context.logger.error(
          `Failed to process ${file}: ${result.error.message}`,
        );
        return result;
      }
...
const withTiming =
  <T>(fn: (options: T, context: CommandContext) => Promise<Result<void>>) =>
