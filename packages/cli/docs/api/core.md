...
const validateUser = createValidationPipeline([
  ...
  createRule(
    "length",
    (value: string) => value.length <= 20 || "Maximum 20 characters",
  ),
  createAsyncRule(
    "unique",
    async (value: string) =>
...
```typescript
function withDefault<T, E>(result: Result<T, E>, defaultValue: T): T {
  return result.success ? result.value : defaultValue;
}

// Usage
const config = withDefault(await loadConfig(), {
  port: 3000,
  host: "localhost",
});
```
  ...
  if (!isOk(fileResult)) {
    return err(
      createError({
        ...
        suggestion:
          'Create server.json with { "host": "localhost", "port": 3000 }',
