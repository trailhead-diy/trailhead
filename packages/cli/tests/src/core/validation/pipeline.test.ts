...
describe('Validation Pipeline', () => {
  describe('Business Logic', () => {
    it('should validate required fields and report all errors', () => {
      const pipeline = createValidationPipeline<{ email: string; age: number }>()
        .add(
          createRule(
            'email',
            'Email is required',
    ...
    it('should continue validation for non-required fields', () => {
      const pipeline = createValidationPipeline<{ name?: string; email?: string }>()
        .add(
          createRule(
            'name',
            'Name format',
  ...
  describe('Async Validation', () => {
    it('should handle async validators correctly', async () => {
      const pipeline = createValidationPipeline<{ apiKey: string }>()
        .add(
          createRule(
            'apiKey',
            'API key validation',
    ...
    it('should process validators sequentially', async () => {
      ...
      const pipeline = createValidationPipeline<{ field1: string; field2: string }>()
        .add(
          createRule(
            'field1',
            'Field 1',
