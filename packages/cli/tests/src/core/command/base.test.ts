...
describe('Command Execution', () => {
  ...
  describe('Command Creation', () => {
    it('should create command with validation and error handling', async () => {
      ...
      const config: CommandConfig<{ test: string }> = {
        ...
        options: [
          { flags: '--test <value>', description: 'Test option', defaultValue: 'default' },
        ],
  ...
  describe('Phase Execution', () => {
    ...
    it('should stop execution on phase failure', async () => {
      const phases: CommandPhase<{}>[] = [
        ...
        {
          name: 'Phase 2',
          execute: async () => Err(createError('PHASE_ERROR', 'Phase 2 failed')),
        },
        {
          name: 'Phase 3',
      ...
      expect(mockContext.logger.error).toHaveBeenCalledWith('Failed at phase: Phase 2')
  ...
  describe('Progress Tracking', () => {
    it('should show progress spinner during execution', async () => {
      ...
      const result = await executeWithProgress(mockTask, 'Processing...', mockContext)
  ...
  describe('Dry Run Mode', () => {
    it('should execute dry run task when enabled', async () => {
      ...
      const result = await executeWithDryRun(realTask, dryRunTask, true, mockContext)

      expect(result.success).toBe(true)
      expect(result.value).toBe('dry run result')
      expect(mockContext.logger.warning).toHaveBeenCalledWith('DRY RUN MODE - No changes will be made')
    ...
    it('should execute real task when dry run is disabled', async () => {
      ...
      const result = await executeWithDryRun(realTask, dryRunTask, false, mockContext)
  ...
  describe('User Interactions', () => {
    it('should display summary with formatted output', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Installation Summary'))
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Files installed'))
