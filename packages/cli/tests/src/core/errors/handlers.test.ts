...
describe('Error Handlers', () => {
  ...
  describe('User-Facing Error Display', () => {
    it('should display errors with helpful suggestions', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Required package not found'))
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Run npm install'))
    })
    ...
    it('should show stack trace in verbose mode for debugging', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Root cause'))
    })
  ...
  describe('Exit Handler', () => {
    it('should exit with correct code for non-recoverable errors', () => {
      ...
      const error = createError('FATAL_ERROR', 'Critical failure', { recoverable: false })
    ...
    it('should always exit regardless of error type', () => {
      ...
      const error = createError('RECOVERABLE_ERROR', 'Can be fixed', { recoverable: true })
  ...
  describe('Retry Mechanism', () => {
    it('should retry failed operations with backoff', async () => {
      ...
      const operation = vi.fn().mockImplementation(async () => {
        ...
        if (attempts < 3) {
          return { 
            success: false, 
            error: createError('TEMP_ERROR', 'Temporary failure', { recoverable: true }) 
          }
        }
    ...
    it('should fail after max retries', async () => {
      const operation = vi.fn().mockResolvedValue({
        success: false,
        error: createError('PERSISTENT_ERROR', 'Persistent failure', { recoverable: true })
      })
    ...
    it('should not retry on non-retryable errors', async () => {
      const operation = vi.fn().mockResolvedValue({
        success: false,
        error: createError('INVALID_INPUT', 'Bad input', { recoverable: false }),
      })
  ...
  describe('Error Recovery', () => {
    it('should attempt recovery for recoverable errors', async () => {
      ...
      const error = createError('RECOVERABLE', 'Can recover', { recoverable: true })
    ...
    it('should not attempt recovery for non-recoverable errors', async () => {
      ...
      const error = createError('FATAL', 'Cannot recover', { recoverable: false })
      const failedResult = { success: false as const, error }
      const result = await tryRecover(failedResult, recoverFn)
