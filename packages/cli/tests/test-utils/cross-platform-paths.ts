/**
 * Cross-Platform Path Testing Utilities for CLI Tests
 *
 * Provides Windows-compatible path handling for CLI tests.
 * Based on the comprehensive utilities from web-ui package.
 */
...
export const createTempPath = (
  ...
  return normalize(
    join(tmpdir(), 'trailhead-cli-tests', `${testName}-${timestamp}`),
  );
};
...
export const testPaths = {
  ...
  mockComponents: isWindows
    ? 'C:\\test\\project\\components'
    : '/test/project/components',
  mockNodeModules: isWindows
    ? 'C:\\test\\project\\node_modules'
...
export const createTestConfig = (overrides: Record<string, any> = {}) => {
  ...
  return {
    ...
    cliRoot: normalize(overrides.cliRoot || baseConfig.cliRoot),
