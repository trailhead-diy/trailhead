...
async function saveUserData(filePath: string, data: string) {
...
async function processIfExists(filePath: string) {
...
async function saveAppState(statePath: string, state: any) {
...
async function setupProjectStructure(projectPath: string) {
...
async function organizeFiles(tempDir: string, finalDir: string) {
...
async function findSourceFiles(projectPath: string) {
...
async function saveReport(reportPath: string, reportData: string) {
...
async function conditionalCopy(srcPath: string, destPath: string, overwrite = false) {
...
async function processConfigFile(configPath: string, outputPath: string) {
...
function analyzeFilePath(inputPath: string) {
...
async function setupProjectLayout(projectName: string, baseDir: string) {
...
async function copyMultipleFiles(fileMappings: Array<{ src: string; dest: string }>) {
...
async function convertDataFiles(inputDir: string, outputDir: string) {
  // Find all data files
...
async function loadUserConfig(configPath: string) {
  // Read config file
  const readResult = await fs.readJson(configPath)
  if (!readResult.success) {
    return readResult
...
- Review [FileSystem API Reference](../reference/api.md) for detailed function documentation
