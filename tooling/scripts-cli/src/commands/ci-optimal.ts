...
import { execCommand, execSequence, createTimer, getElapsedSeconds } from '../utils/subprocess.js'
  ...
  action: async (options, context): Promise<Result<void, CoreError>> => {
    ...
    try {
      ...
      if (!options.skipDocs) {
        ...
        const docsResult = await execCommand(
          'pnpm', 
          ['docs:validate'],
          context,
          { allowFailure: true }
        ...
        if (docsResult.isErr()) {
          context.logger.warning(colorize('yellow', withIcon('warning', 'Documentation validation failed (non-blocking)')))
        }
      }
      ...
      if (!options.skipSecurity) {
        ...
        if (auditResult.isErr()) {
          context.logger.warning(colorize('yellow', withIcon('warning', 'Security vulnerabilities found (non-blocking)')))
        }
      }

      // 6. Bundle size check (if size-limit is available)
      const sizeLimitResult = await execCommand(
        'which',
        ['size-limit'],
        context,
        { allowFailure: true }
      ...
      if (sizeLimitResult.isOk()) {
        ...
        if (sizeResult.isErr()) {
          context.logger.warning(colorize('yellow', withIcon('warning', 'Bundle size limits exceeded')))
        }
      }
      ...
      context.logger.info('')
      context.logger.info(colorize('blue', border))
      context.logger.info(colorize('green', withIcon('success', `All checks passed in ${duration}s`)))
      context.logger.info(colorize('blue', border))
      ...
      context.logger.info('')
      context.logger.info(colorize('green', withIcon('money', `Estimated savings: ${minutesSaved} GitHub Actions minutes ($${savings})`)))
