    ...
    try {
      ...
      if (options.fix && (combinedResult.errors > 0 || combinedResult.warnings > 0)) {
        ...
        if (fixResult.isErr()) {
          ...
          context.logger.info(colorize('green', withIcon('success', 'Fixed all auto-fixable issues')))
        }
      }
...
const validateTurboJson = async (context: CommandContext): Promise<Result<ValidationResult, CoreError>> => {
  ...
  try {
    ...
    if (testTask) {
      ...
      if (!hasVitestDep) {
        ...
        if (context.verbose) {
          context.logger.info(colorize('green', '  ✅ Test tasks correctly depend on @repo/vitest-config#build'))
        }
      }
    }
...
const displayValidationSummary = (result: ValidationResult, context: CommandContext): void => {
  ...
  if (result.errors === 0 && result.warnings === 0) {
    ...
    if (result.warnings > 0) {
      context.logger.info(colorize('yellow', withIcon('warning', `Found ${result.warnings} warnings`)))
    }
    if (result.errors > 0) {
...
const displayDependencyGraph = async (packageDirs: readonly PackageInfo[], context: CommandContext): Promise<void> => {
  context.logger.info('')
    ...
    try {
      ...
      for (const [depName, depVersion] of Object.entries(allDeps)) {
        if (typeof depVersion === 'string' && 
            (depVersion.startsWith('workspace:') || depName.startsWith('@repo/') || depName.startsWith('@esteban-url/'))) {
  ...
  const printDependencyTree = (pkgName: string, prefix = '', isLast = true) => {
    if (visited.has(pkgName)) {
      context.logger.info(`${prefix}${isLast ? '└── ' : '├── '}${pkgName} ${colorize('yellow', '(circular)')}`)
      return
    }
      ...
      if (contentResult.isOk()) {
        try {
          ...
          if (!turboConfig.tasks.test.dependsOn.includes('@repo/vitest-config#build')) {
            ...
            if (writeResult.isOk()) {
              context.logger.info(colorize('green', '  ✅ Added @repo/vitest-config#build dependency to test tasks'))
              fixedCount++
            } else {
              failedCount++
            }
  ...
  if (failedCount > 0) {
    return err(
      createCoreError(
        'FIX_FAILED',
        'FILE_SYSTEM_ERROR',
        `Failed to fix ${failedCount} issues`,
        {
          recoverable: true,
